...
if not DISCORD_TOKEN or not APIFY_TOKEN:
    print("❌ Missing environment variables: DISCORD_TOKEN or APIFY_TOKEN")
    exit(1)
...
async def help_command(ctx):
    embed = discord.Embed(
        ...
        description="Check if a TikTok video is shadow-banned",
        color=0x00ff00
    )
    embed.add_field(
        name="📊 Commands",
...
async def check_video(ctx, url: str = None):
    ...
    async with ctx.typing():
        try:
            ...
            error_msg = str(e)[:200]
            if "403" in error_msg:
                await ctx.send("❌ **TikTok is blocking this request.**\nTry:\n• Use a full URL (not short link)\n• Wait 5-10 minutes and retry")
            else:
